# -*- coding: utf-8 -*-
"""修复 scenic_spot 表的 province 字段 - 城市→省份映射"""
...
CITY_TO_PROVINCE = {
    ...
    '乌兰察布': '内蒙古', '兴安盟': '内蒙古', '锡林郭勒': '内蒙古', '阿拉善': '内蒙古',
    '阿拉善盟': '内蒙古', '满洲里': '内蒙古', '二连浩特': '内蒙古',
    # 辽宁
    ...
    '定西': '甘肃', '陇南': '甘肃', '临夏': '甘肃', '甘南': '甘肃', '敦煌': '甘肃',
    # 青海
    '西宁': '青海', '海东': '青海', '海北': '青海', '黄南': '青海', '海南州': '青海',
    '果洛': '青海', '玉树': '青海', '海西': '青海', '海西州': '青海', '海北州': '青海',
    '海南': '青海',
    # 宁夏
    '银川': '宁夏', '石嘴山': '宁夏', '吴忠': '宁夏', '固原': '宁夏', '中卫': '宁夏',
    # 新疆
    '乌鲁木齐': '新疆', '克拉玛依': '新疆', '吐鲁番': '新疆', '哈密': '新疆',
    '昌吉': '新疆', '博州': '新疆', '库尔勒': '新疆', '阿克苏': '新疆',
    '克州': '新疆', '喀什': '新疆', '和田': '新疆', '伊犁': '新疆', '塔城': '新疆',
...
# 获取所有需要修复的 province 记录
cities_raw = run_sql("SELECT DISTINCT city FROM scenic_spot WHERE province = city AND city NOT IN ('北京','上海','天津','重庆') ORDER BY city")
cities = [c.strip() for c in cities_raw.split('\n') if c.strip()]

print(f"需要修复的城市: {len(cities)} 个")
fixed = 0
unknown = []

for city in cities:
    if city in CITY_TO_PROVINCE:
        correct_province = CITY_TO_PROVINCE[city]
        sql = f"UPDATE scenic_spot SET province='{correct_province}' WHERE city='{city}' AND province='{city}'"
        run_sql(sql)
        fixed += 1
        print(f"  ✓ {city} → {correct_province}")
    else:
        unknown.append(city)

if unknown:
    print(f"\n未找到映射的城市 ({len(unknown)}):")
    for c in unknown:
        print(f"  ✗ {c}")

# 验证修复结果
result = run_sql("SELECT province, COUNT(DISTINCT city) as city_count, COUNT(*) as spot_count FROM scenic_spot GROUP BY province ORDER BY province")
print(f"\n修复后省份统计:")
